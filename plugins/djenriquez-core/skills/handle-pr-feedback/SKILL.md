...
- **Required humanizer pass:** before posting each reply, apply `djenriquez-core:humanizer` in `pr-reply` mode (Claude: `/humanizer` or load the skill + `references/humanizer-patterns.md`; Codex: read the installed skill). Keep replies to one or two factual sentences.
