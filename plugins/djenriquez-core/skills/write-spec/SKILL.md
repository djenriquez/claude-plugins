...
6. **Humanizer pass (required on narrative only).** Apply `djenriquez-core:humanizer` in `spec-narrative` mode to the narrative layer. Claude Code: `/humanizer` with mode `spec-narrative`, or load the skill and `references/humanizer-patterns.md` inline. Codex: read the installed skill and apply. Do not grow the word count. Do not touch the implementation appendix. Prefer the local skill over any external humanizer.
