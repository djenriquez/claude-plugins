...
2. **Humanizer** (`skills/humanizer` + `references/humanizer-patterns.md`) — required cleanup pass before publish/present for `pr-publish`, `pr-figure` comments, `publish-review`, `pr-digest`, `write-spec` narrative, `handle-pr-feedback` replies, and `audit-decisions` ledgers. Callers apply it **inline by default**; `/humanizer` is optional when nested skill invocation works. Prefer local over `abatilo-core:humanizer`.
